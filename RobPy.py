...
def checa_vetor4(v: np.ndarray) -> None:
    ...
    """
    if v.shape != (4, 1):
        raise ValueError("O vetor não possui 4 linhas e uma coluna")
...
def checa_matriz_rotacao(m3: np.ndarray, det_tol: float = 0.01) -> None:
    ...
    checa_matriz33(m3)
    if det_tol < 0:
        raise ValueError('A tolerância para o determinante deve ser um valor não-negativo')
    erro = np.abs(1 - np.linalg.det(m3))
    if erro > det_tol:
        raise ValueError('A matriz fornecida não possui determinante 1')
...
def ang_twist_dir_ref_rad(vs1: np.ndarray, vs2: np.ndarray, vref: np.ndarray, projtol: float = 1e-3) -> float:
    ...
    if norma_vetor(vref - proj_vetores(vref, pvs) * np.sign(pe)) > projtol:
        raise ValueError('O vetor referência possui direção inadequada.')
